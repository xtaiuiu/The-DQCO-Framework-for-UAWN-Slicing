...
def run(n_repeats=20):
    thetas = np.arange(2, 11) * 2

    df_rate_avg = pd.DataFrame(
        {'BCD': np.zeros(len(thetas)), 'PSO': np.zeros(len(thetas)), 'SHIO': np.zeros(len(thetas))})
    for repeat in range(n_repeats):
        print(f" ####################### repeat = {repeat} ##############################")

        rate_pso_arr, rate_shio_arr, rate_bcd_arr = [], [], []
        for theta in thetas:
            print(f" ####################### repeat = {repeat}, bandwidth = {theta} ##############################")
            logging.basicConfig(level=logging.WARNING)
            np.set_printoptions(formatter={'float': '{:0.4f}'.format})
            sc = create_scenario(50, theta)
            sc.pn.b_tot = 100
            sc.pn.p_max = 50
    ...
    df_rate_avg.to_excel('objective_value_radius_jnl.xlsx')
    df_rate_avg.plot()
    plt.show()
...
def load_and_plot():
    fontsize = 20
    df_obj_avg = pd.read_excel('objective_value_radius_jnl.xlsx', index_col=0)
    df_obj_avg.columns = [r'$BCD^2$', 'AIW-PSO', 'SHIO']
    G = np.arange(df_obj_avg.shape[0])
    plt.rcParams.update({'font.size': fontsize})

    ax = df_obj_avg.plot(legend=True, lw=2, xlabel=r'$Network radius \times 10^2 m$', ylabel='Max-min user rate (Mbps)',
                         fontsize=fontsize, style=["r-s", "m-.d", "c-->"], markersize=10, grid=True,
                         markerfacecolor='none')
    ax.set_xticks(G)
    # plt.ylim((0.295, 0.5))
    ax.set_xticklabels([str((i + 2) * 2) for i in G], fontsize=fontsize)
    plt.show()
