...
def run(n_repeats=20):
    bandwidths = np.arange(2, 9) * 25

    df_rate_avg = pd.DataFrame(
        {'BCD': np.zeros(len(bandwidths)), 'PSO': np.zeros(len(bandwidths)), 'SHIO': np.zeros(len(bandwidths))})
    for repeat in range(n_repeats):
        print(f" ####################### repeat = {repeat} ##############################")

        rate_pso_arr, rate_shio_arr, rate_bcd_arr = [], [], []
        for bandwidth in bandwidths:
            print(f" ####################### repeat = {repeat}, bandwidth = {bandwidth} ##############################")
            logging.basicConfig(level=logging.WARNING)
            np.set_printoptions(formatter={'float': '{:0.4f}'.format})
            sc = create_scenario(50, 10)
            sc.pn.b_tot = bandwidth
            sc.pn.p_max = 50
    ...
    df_rate_avg.to_excel('objective_value_bandwidths_jnl.xlsx')
    df_rate_avg.plot()
    plt.show()
...
def load_and_plot():
    fontsize = 20
    df_obj_avg = pd.read_excel('objective_value_bandwidths_jnl.xlsx', index_col=0)
    df_obj_avg.columns = [r'$DQCO$', 'SCA', 'MADDPG']
    G = np.arange(df_obj_avg.shape[0])
    plt.rcParams.update({'font.size': fontsize})

    ax = df_obj_avg.plot(legend=True, lw=2, xlabel='Total bandwidth (MHz)', ylabel='Max-min user rate (Mbps)',
                         fontsize=fontsize, style=["r-s", "m-.d", "c-->"], markersize=10, grid=True,
                         markerfacecolor='none')
    ax.set_xticks(G)
    # plt.ylim((0.295, 0.5))
    ax.set_xticklabels([str((i + 2) * 25) for i in G], fontsize=fontsize)
    plt.show()
